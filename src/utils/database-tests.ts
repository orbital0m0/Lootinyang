...
export const testHabitOperations = async (userId: string) => {
  try {
    ...
    // Read
    const { data: readHabits, error: readError } = await supabase
      .from('habits')
      .select('*')
      .eq('user_id', userId);
...
export const testDailyCheckOperations = async (habitId: string) => {
  try {
    ...
    // Read
    const { data: readChecks, error: readError } = await supabase
      .from('daily_checks')
      .select('*')
      .eq('habit_id', habitId);
...
// 종합 테스트 함수
export const runAllDatabaseTests = async (): Promise<TestResults> => {
  ...
  try {
    ...
    results.errors.push(`예상치 못한 오류: ${error instanceof Error ? error.message : String(error)}`);
  }
